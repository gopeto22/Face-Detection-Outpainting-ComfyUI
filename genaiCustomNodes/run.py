...
import torch
import numpy as np
from genaiCustomNodes import (
    GENAI_LoadYuNetModel,
    GENAI_DetectFacesYuNet,
    GENAI_VisualizeYuNetLandmarks,
    GENAI_CropImageByBoundingBox,
...
if __name__ == "__main__":
    ...
    yunet_model = GENAI_LoadYuNetModel().run("/path/to/your/yunet_model.onnx")[0]

    for img_file in os.listdir(stage_2_input_dir):
        img_path = os.path.join(stage_2_input_dir, img_file)
        img = Image.open(img_path)
        face_detections, bounding_boxes = GENAI_DetectFacesYuNet.detect_faces(yunet_model, img, 1.0)
        if bounding_boxes:
            img_with_landmarks, bounding_boxes = GENAI_VisualizeYuNetLandmarks.visualize_landmarks(img, face_detections, 2)
            img_with_landmarks.save(os.path.join(stage_2_output_dir, img_file))
        else:
            print(f"No valid bounding boxes found for {img_file}")
